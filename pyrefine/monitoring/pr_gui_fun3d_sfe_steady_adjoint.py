...
from pyrefine.post_processing.sfe_file_reader import (
    SFEForwardHistoryReader, SFEGoalOrientedHistoryReader)
...
class GuiAdaptHistory(PyrefinePlotly):
    def __init__(self):
        ...
        self.full_layout = html.Div(children=sections, style=dict(backgroundColor=self.background_color))
    ...
    def create_case_information_div(self):
        ...
        children = [html.H1("Case Information"), self.create_case_information_input_table(), load_button]
            ...
            [
                html.Tr(
                    [
                        ...
                        dcc.Input(id="data_directory", type="text", value="./Flow", style=dict(width="300%")),
                    ]
                ),
                html.Tr(
                    [
                        html.Td("project_rootname:"),
                        dcc.Input(id="project_rootname", type="text", value="om6ste", style=dict(width="300%")),
                    ]
                ),
            ]
        )
    ...
    def create_graphs_div(self):
        ...
        div = html.Div(children=children, id="div_adapt_hist")
        return div

    def generate_adaptation_history_div(self):
        self.hist_fig = self.generate_adapt_history_fig()
        export_html = self.generate_export_field_and_button(
            default_filename="adapt_hist.html", button_txt="Export interactive figure", id_base="hist_export_html"
        )
        export_tec = self.generate_export_field_and_button(
            default_filename="adapt_hist.dat", button_txt="Export tecplot data", id_base="hist_export_tec"
        )
        children = [html.H1("Adaptation History"), dcc.Graph(figure=self.hist_fig, id="adapt_hist_graph")]
        ...
        for variable_name, var_data in self.reader.final_hist_values.items():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    hovertemplate=("Mesh: %{customdata[0]:n}<br>" + "Nodes: %{x:d} <br>" + "Value: %{y:.3e}"),
                )
            )
    ...
    def generate_plots_tabs(self):
        tabs = dcc.Tabs(
            [
                dcc.Tab(
                    label="Convergence", children=[self.generate_sfe_forward_convergence_div()], id="convergence_tab"
                ),
                dcc.Tab(label="Timing", children=[self.create_sfe_forward_timing_div()], id="timing_tab"),
                dcc.Tab(
                    label="Preconditioning",
                    children=[self.generate_sfe_forward_preconditioning_div()],
                    id="preconditioning_tab",
                ),
    ...
    def create_sfe_forward_timing_div(self):
        ...
        div = html.Div(
            children=[
                html.H2("SFE Forward Solve Timing"),
    ...
    def generate_sfe_forward_timing_div(self):
        ...
        export_html = self.generate_export_field_and_button(
            default_filename="sfe_forward_timing.html",
            button_txt="Export interactive figure",
            id_base="sfe_forward_timing_export_html",
        )
        export_tec = self.generate_export_field_and_button(
            default_filename="sfe_forward_timing.dat",
            button_txt="Export tecplot data",
    ...
    def generate_sfe_forward_preconditioning_div(self):
        ...
        export_html = self.generate_export_field_and_button(
            default_filename="sfe_forward_preconditioning.html",
            button_txt="Export interactive figure",
            id_base="sfe_forward_preconditioning_export_html",
        )
        export_tec = self.generate_export_field_and_button(
            default_filename="sfe_forward_preconditioning.dat",
            button_txt="Export tecplot data",
            id_base="sfe_forward_preconditioning_export_tec",
        )
        children = [
            html.H2("SFE Forward Preconditioning"),
    ...
    def generate_sfe_adjoint_div(self):
        ...
        export_html = self.generate_export_field_and_button(
            default_filename="sfe_adjoint_hist.html",
            button_txt="Export interactive figure",
            id_base="sfe_adjoint_export_html",
        )
        export_tec = self.generate_export_field_and_button(
            default_filename="sfe_adjoint_hist.dat", button_txt="Export tecplot data", id_base="sfe_adjoint_export_tec"
        )
        children = [html.H2("SFE Adjoint History"), dcc.Graph(figure=self.adjoint_hist_fig, id="adjoint_hist_graph")]
        children.extend(export_html)
        children.extend(export_tec)
    ...
    def generate_sfe_forward_convergence_fig(self):
        ...
        for variable_name, var_data in self.sfe_fwd_reader.residual_convergence_data.items():
            vis = (
                None
                if variable_name in ["Density", "X-momentum", "Y-momentum", "Z-momentum", "Energy", "Turbulence"]
                else "legendonly"
            )
            ...
            fig_sfe_fwd_convergence.add_trace(
                go.Scatter(
                    ...
                    name=variable_name,
                    visible=vis,
                    hovertemplate=("Steps: %{x:d} <br>" + "Value: %{y:.3e}"),
                ),
                secondary_y=sec_y,
            )
        xaxis, yaxis = self.get_axis_settings()
...
def main():
    ...
    @app.callback(
        [Output("div_adapt_hist", "children"), Output("sfe_detail_plots_div", "children")],
        [Input("load_button", "n_clicks")],
        [State("data_directory", "value"), State("project_rootname", "value")],
    )
