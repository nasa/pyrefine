...
class GuiAdaptHistoryLive(PyrefinePlotly):
    ...
    def create_case_information_div(self):
        start_button = html.Button(
            "Start", id="start_button", n_clicks=0, style=dict(backgroundColor="lightblue", fontSize="24px")
        )
        children = [
            html.H1("Case Information"),
    ...
    def create_case_information_input_table(self):
        return html.Table(
            [
                html.Tr(
                    [
                        html.Td("Refresh interval in minutes:"),
                        dcc.Input(id="refresh_interval_input", type="number", value=0.5, style=dict(width="30%")),
                    ]
                ),
                html.Tr(
                    [
    ...
    def create_plot_settings_input_table(self):
        return html.Table(
            [
                html.Tr(
                    [
                        html.Td("Active variables:"),
                        dcc.Input(id="variable_list", type="text", value="C_L, C_D", style=dict(width="300%")),
                    ]
                ),
                html.Tr(
                    [
                        html.Td("Use log scale:"),
                        dcc.Checklist(
                            id="log_scale_checklist",
                            options=[dict(label="X axis", value="xlog"), dict(label="Y axis", value="ylog")],
