...
class AFLR3:
    ...
    def run(self):
        ...
        print(f'Initial Wall Spacing, Number of Layers, BL Growth Rate = {self.initial_wall_spacing}, {self.nbl}, {self.max_blgr}')
        ...
        -blrm {self.max_blgr} -tmp $PWD -log {self.aflr_extra_args}', shell=True)
        ...
        \ncd ../hybrid \
        \nref translate {self.project_name}.lb8.ugrid {self.project_name}01.meshb',  shell=True)
        os.chdir('../hybrid')

    def compute_spacing(self):
        if (self.bl_type == 'manual'):
            if (self.nbl == 0 or self.initial_wall_spacing == None):
                raise ValueError("""nbl and/or initial_wall_spacing undefined.
        ...
        if iterations == 1:
            ...
            raise ValueError("""Could not find yplus or re_cell output.
